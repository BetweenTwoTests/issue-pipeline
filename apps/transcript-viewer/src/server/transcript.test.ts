...
import { parseLines, parseTranscript } from "./transcript";

/** Fixture lines mirror the REAL shapes observed in Claude Code's session
 * store (~/.claude/projects/...) -- if a claude release changes the format,
 * update these from a fresh real file. */
const LINES = [
  { type: "queue-operation", operation: "enqueue", content: "ignored" },
  { type: "ai-title", aiTitle: "Create issue-1.md with heading", sessionId: "x" },
  { type: "custom-title", customTitle: "Issue file work", sessionId: "x" },
  {
    type: "user",
    uuid: "u-1",
...
test("parseTranscript extracts prompt, thinking, text, tool_use, and tool_result in order", () => {
  ...
  assert.equal(parsed.aiTitle, "Create issue-1.md with heading");
  assert.equal(parsed.customTitle, "Issue file work");
  ...
  assert.equal(toolUse.toolUseId, "toolu_1");
  assert.equal(toolResult.toolUseId, "toolu_1");
  ...
  assert.equal(sidechainResult.agentId, "agent-7");
...
test("parseLines namespaces fallback ids for entries without a uuid", () => {
  const lineJson = JSON.stringify({
    type: "assistant",
    message: { role: "assistant", content: [{ type: "text", text: "hi" }] },
  });
  assert.equal(parseLines([lineJson], "100").events[0].id, "100:0:0");
  assert.equal(parseLines([lineJson], "200").events[0].id, "200:0:0");
});
...
test("oversized event text is truncated with a marker", () => {
  const big = "x".repeat(200_001);
  const parsed = parseTranscript(JSON.stringify({ type: "user", message: { role: "user", content: big } }));
  const text = parsed.events[0].text ?? "";
  assert.ok(text.length < big.length + 100);
  assert.ok(text.endsWith("… [truncated 1 more characters]"));
});
