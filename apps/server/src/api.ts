...
import { getPipeline, listPipelines, signalAnswers, signalControl, startPipeline, TEMPORAL_UNCONFIGURED } from "./pipelines";
...
/** Unconfigured env -> 503, unknown workflow/issue -> 404, else 500. */
function pipelineErrorStatus(err: unknown): number {
  if (err instanceof Error && (err.message === TEMPORAL_UNCONFIGURED || err.message === PIPELINE_CONFIG_UNSET)) {
    return 503;
  }
  const name = (err as { name?: string })?.name ?? "";
  if (name === "WorkflowNotFoundError" || name === "TrackerIssueNotFoundError") return 404;
...
export function createApiMiddleware(root: string = defaultRoot()) {
  return async function transcriptApi(req: IncomingMessage, res: ServerResponse, next: () => void): Promise<void> {
    ...
    try {
      if (
        url.pathname === "/api/pipeline/answer" ||
        url.pathname === "/api/pipeline/control" ||
        url.pathname === "/api/pipeline/start" ||
        (url.pathname === "/api/issues" && req.method === "POST") ||
        url.pathname === "/api/issue/comment"
      ) {
        if (req.method !== "POST") {
        ...
        try {
          if (url.pathname === "/api/pipeline/start") {
            const parsed = parseStartRequest(body);
            if (!parsed.ok) {
              send(400, { error: parsed.error });
              return;
            }
            send(200, await startPipeline(parsed.value.ref, process.env.USER ?? "unknown"));
            return;
          }
          if (url.pathname === "/api/issues") {
            const parsed = parseCreateIssueRequest(body);
            if (!parsed.ok) {
              send(400, { error: parsed.error });
              return;
            }
            send(200, { issue: await createIssue(parsed.value) });
            return;
          }
          if (url.pathname === "/api/issue/comment") {
            const parsed = parseIssueCommentRequest(body);
            if (!parsed.ok) {
              send(400, { error: parsed.error });
              return;
            }
            send(200, {
              comment: await addHumanComment(parsed.value.key, parsed.value.body, process.env.USER ?? "human"),
            });
            return;
          }
      ...
      if (url.pathname === "/api/issues") {
        try {
          send(200, { issues: await listIssues() });
        } catch (err) {
          sendPipelineError(err);
        }
        return;
      ...
      if (url.pathname === "/api/issue") {
        const parsed = parseIssueKeyParams(url.searchParams.get("repo") ?? "", url.searchParams.get("number") ?? "");
        if (!parsed.ok) {
          send(400, { error: parsed.error });
          return;
        }
        try {
          const detail = await getIssueDetail(parsed.value);
          if (!detail) {
            send(404, { error: `no tracker issue ${parsed.value.repoOwner}/${parsed.value.repoName}#${parsed.value.number}` });
            return;
          }
          send(200, detail);
        } catch (err) {
      ...
      if (url.pathname === "/api/repos") {
        try {
          send(200, { repos: await listRegisteredRepos() });
        } catch (err) {
          sendPipelineError(err);
        }
        return;
