...
import type { PipelineListItem } from "@issue-pipeline/server";
import { fetchPipeline, sendAnswers, sendControl } from "./api";
import { formatWhen } from "./format";
import { issueHash } from "./routes";
...
const DEAD_EXECUTION_STATUSES = new Set(["FAILED", "TERMINATED", "TIMED_OUT", "CANCELLED"]);

export function statusLabel(item: PipelineListItem): string {
  // A dead execution outranks the plan snapshot: a workflow that failed
  // mid-planning would otherwise keep showing "planning" forever.
  const label = DEAD_EXECUTION_STATUSES.has(item.executionStatus)
    ? item.executionStatus
    : (item.plan?.status ?? item.executionStatus);
...
export function PipelineView({ workflowId }: { workflowId: string }) {
  ...
  const issueUrl = plan ? issueHash(plan.owner, plan.repo, plan.issueNumber) : null;
    ...
    <div className="transcript">
      <header className="transcript-head">
        ...
        <div className="chips">
          ...
          {issueUrl ? (
            <a className="chip chip-link" href={issueUrl}>
              issue ↗
            </a>
          ) : null}
          ...
          <section className="card pipe-section">
            ...
            {plan.phases.map((p, i) => (
              <div key={p.subIssueNumber} className="phase-row">
                ...
                <a className="chip chip-link" href={issueHash(plan.owner, plan.repo, p.subIssueNumber)}>
                  #{p.subIssueNumber}
                </a>
